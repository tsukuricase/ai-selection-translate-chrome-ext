...
function showPopover(x, y, text) {
  ...
  setTimeout(() => {
    ...
    let nx = x, ny = y;
    if (rect.right > window.innerWidth) nx -= (rect.right - window.innerWidth + 10);
    if (rect.bottom > window.innerHeight) ny -= (rect.bottom - window.innerHeight + 10);
...
function translateWithAI(text, apiKey, apiType) {
  if (!apiKey || !apiType) {
    if (popover) popover.querySelector('span').innerText = '请先在插件设置API KEY';
    return;
  }
  ...
  chrome.runtime.sendMessage(
    ...
    (response) => {
      ...
      if (chrome.runtime.lastError)
        popover.querySelector('span').innerText = '通信失败: ' + chrome.runtime.lastError.message;
